...
struct MtNode                   *mt_root;       /* pointer to the root node of the binary tree */
...
void parse_rules(char *in_fn, MtNode *root){
    FILE        *fp;
...
void my_callback(u_char *user, const struct pcap_pkthdr *pkthdr, const u_char *pktdata)
{
    ...
    verdict = lookup(mt_root, dst_addr);
